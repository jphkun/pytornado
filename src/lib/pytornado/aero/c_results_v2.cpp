                 ...
                 refstruct* refs, resultstruct* results, long int num_pan)
{
    ...
    alpha = deg2rad(state->alpha);
    beta = deg2rad(state->beta);


    get_freestream_direction(freestream_dir, alpha, beta);

    // Velocity of aircraft opposite to freestream direction
    vel_wind_x = -state->airspeed*freestream_dir[0];
    vel_wind_y = -state->airspeed*freestream_dir[1];
    vel_wind_z = -state->airspeed*freestream_dir[2];
    ...
    for (int i = 0 ; i < num_pan ; ++i)
    {
        ...
        index_VA = i*NUM_V*NUM_COORD + 1*NUM_COORD;
        index_VB = i*NUM_V*NUM_COORD + 2*NUM_COORD;

        // Midpoint of bound vortex filament
        P_x = lattice->C[index_i    ];
        P_y = lattice->C[index_i + 1];
        P_z = lattice->C[index_i + 2];
        ...
        for (int j = 0 ; j < num_pan ; ++j)
        {
            ...
            for (int k = 0 ; k < NUM_V - 1 ; ++k)
            {
                ...
                index_V1 = j*NUM_V*NUM_COORD + k*NUM_COORD;
                index_V2 = j*NUM_V*NUM_COORD + (k + 1)*NUM_COORD;

                // CALCULATE VECTORS R0, R1, R2 ******************************/
                r1_x = P_x - lattice->V[index_V1    ];
                r1_y = P_y - lattice->V[index_V1 + 1];
                r1_z = P_z - lattice->V[index_V1 + 2];
                r1_mag = sqrt(r1_x*r1_x + r1_y*r1_y + r1_z*r1_z);

                r2_x = P_x - lattice->V[index_V2    ];
                r2_y = P_y - lattice->V[index_V2 + 1];
                r2_z = P_z - lattice->V[index_V2 + 2];
                r2_mag = sqrt(r2_x*r2_x + r2_y*r2_y + r2_z*r2_z);

                // Dotproduct r1*r2
                r1r2_dot = r1_x*r2_x + r1_y*r2_y + r1_z*r2_z;

                /* CROSS-PRODUCT R1 x R2 *************************************/
                rcross_x = r1_y*r2_z - r1_z*r2_y;
                rcross_y = r1_z*r2_x - r1_x*r2_z;
                rcross_z = r1_x*r2_y - r1_y*r2_x;

                /* INDUCED VELOCITY FOR UNIT VORTEX STRENGTH *****************/
                // Mod JP
                v_c = (1/(4.0*M_PI))*(1/(r1r2_dot + r1_mag*r2_mag*(1 + lattice->EPSILON)))*(1/r1_mag + 1/r2_mag);
                v_x = v_c*rcross_x;
                v_y = v_c*rcross_y;
                v_z = v_c*rcross_z;
